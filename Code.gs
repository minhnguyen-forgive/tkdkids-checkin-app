...
/**
 * Phục vụ HTTP GET (Cho cả Web App lẫn REST API trên GitHub Pages)
 */
function doGet(e) {
  // Trả về JSON cho GitHub Pages nếu có parameter action=getInitialData
...
/**
 * Phục vụ HTTP POST (Cho GitHub Pages thực hiện Check-in & Đổi Nguồn)
 */
function doPost(e) {
  try {
    let contents = {};
...
function updateSpreadsheetSource(newUrlOrId, newSheetName) {
  try {
    ...
    return { success: false, message: 'Không thể mở file Google Sheet mới. Vui lòng kiểm tra quyền chia sẻ: ' + err.message };
  }
}
